...
# Iterate over each self-managed node group and attach policies dynamically
locals {
  # Extract IAM role names from all self-managed node groups
  node_group_roles = [
    for ng_name, ng_details in module.eks_cluster.self_managed_node_groups : ng_details.iam_role_name
  ]
}
